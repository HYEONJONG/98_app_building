...
def df_filter(message, df):
    slider_1, slider_2 = st.slider('%s' % (message), 0, len(df) - 1, [len(df) - 155, len(df) - 1], 1)
...
if option == 'Global Risk Board':
    ...
    st.sidebar.markdown(
        '매크로, 주식/금리/FX에 대한 리스크 요인을 감안해 자산시장의 전반적인 위기수준을 판단하기 위한 리스크 종합지표(RCI: Risk Composite Index). RCI는 직전 3년 기간 대비 리스크 요인의 percent rank 변화를 확인해 현재 시점이 어느 수준인지를 측정. 0~1 사이에서 움직이며 0.5가 평균. RCI가 높아질 수록 위기에 가까워짐을 의미')
    st.sidebar.text("")
    st.sidebar.text("")
    ...
    df[['GRCI_안정국면', 'GRCI_위기국면', 'KRCI_안정국면', 'KRCI_위기국면']] = df[
        ['GRCI_안정국면', 'GRCI_위기국면', 'KRCI_안정국면', 'KRCI_위기국면']].replace("  ", 0)
    df[['GRCI_안정국면', 'GRCI_위기국면', 'KRCI_안정국면', 'KRCI_위기국면']] = df[
        ['GRCI_안정국면', 'GRCI_위기국면', 'KRCI_안정국면', 'KRCI_위기국면']].replace("1", 1)

    st.header('Global Risk Board')
    st.markdown(
        '리스크 스코어보드는 국내와 글로벌 자산시장의 전반적인 위기수준을 나타냅니다. 매크로와 함께 주식, 금리, 외환시장 등 22개 세부 리스크 요인을 확인할 수 있도록 만든 스코어보드를 작성하고, 세부 리스크 요인을 종합해 산출한 리스크 종합지수를 통해 전반적인 위기 수준을 점검합니다. 글로벌과 국내 리스크를 구분해 GRCI와 KRCI를 주간 단위로 제시하고 있습니다.')
    ...
    st.markdown('%s 기준' % (end_date.item().strftime("%m/%d/%Y")))

    perc = filtered_df.iloc[start_index:end_index, 1:25].rank(pct=True).tail(1) * 100
    perc = perc.T
    perc["var"] = perc.index
    perc["avg"] = 50
    ...
    d = alt.Chart(perc).mark_bar().encode(
        ...
        color=alt.condition(
            ...
            alt.value("orange"),
            alt.value("steelblue"))).interactive()
    st.altair_chart(d, use_container_width=True)

    # Sub risk Index
    st.subheader('3. 세부 리스크 지표')
    ...
    chart = alt.Chart(filtered_df).encode(x=alt.X('Date', title=None),
                                          y=alt.Y(tickerSymbol, scale=alt.Scale(zero=False))).interactive()
    ...
    b = alt.Chart(filtered_df).mark_line().encode(x=alt.X('Date', title=None), y='GRCI',
                                                  color=alt.value('#d62728')).interactive()
    c = alt.Chart(filtered_df).mark_line().encode(x=alt.X('Date', title=None), y='KOSPI').interactive()
    d = alt.Chart(filtered_df).mark_line().encode(x=alt.X('Date', title=None), y='KRCI',
                                                  color=alt.value('#d62728')).interactive()
    e = alt.Chart(filtered_df).mark_area(opacity=0.5, color="grey").encode(x=alt.X('Date', title=None),
                                                                           y=alt.Y('GRCI_안정국면',
                                                                                   title=None)).interactive()
    f = alt.Chart(filtered_df).mark_area(opacity=0.5, color="#d62728").encode(x=alt.X('Date', title=None),
                                                                              y=alt.Y('GRCI_위기국면', title=None),
                                                                              color=alt.value('red')).interactive()
    g = alt.Chart(filtered_df).mark_area(opacity=0.5, color="grey").encode(x=alt.X('Date', title=None),
                                                                           y=alt.Y('KRCI_안정국면',
                                                                                   title=None)).interactive()
    h = alt.Chart(filtered_df).mark_area(opacity=0.5, color="#d62728").encode(x=alt.X('Date', title=None),
                                                                              y=alt.Y('KRCI_위기국면', title=None),
                                                                              color=alt.value('red')).interactive()

    st.subheader(f'4. {opt}와 주가지수 추이')
    if opt == 'GRCI (Global Risk Composite Index)':
    ...
    # Regime vs RCI
    st.subheader(f'5. {opt}와 리스크 국면')
    st.markdown(
        '통계적인 확률 국면분석 모형(hiddem Markov model)을 활용하여 리스크에 대한 국면을 구분합니다. RCI의 변화에 따라 안정(회색), 전환(흰색), 위기(빨강)의 3국면을 구분하여 나타냅니다')
...
if option == 'Sentiment Board':
    st.sidebar.markdown('**Methodology**')
    st.sidebar.markdown(
        '2008년 이후 매월마다 국내 증권사의 전체 기업분석 리포트의 텍스트를 분석해 여기에 사용된 단어의 감성을 긍정/부정/중립/불확실성의 4가지 카테고리로 구분합니다. 실적시즌에 따른 계절성을 감안해 발간건수를 가중한 4개월 이동평균을 사용해 감성지수를 산출합니다.')
    st.sidebar.text("")
    st.sidebar.text("")
    ...
    chart = alt.Chart(filtered_df).encode(x=alt.X('Date', title=None), y='감성지수').interactive()
    st.altair_chart(chart.mark_line(), use_container_width=True)
    st.markdown(download_csv('Filtered Data Frame', filtered_df), unsafe_allow_html=True)
    ...
    a = alt.Chart(filtered_df).encode(x=alt.X('Date', title=None), y='긍정적').interactive()
    b = alt.Chart(filtered_df).encode(x=alt.X('Date', title=None), y='부정적', color=alt.value('red')).interactive()
    ...
    a = alt.Chart(filtered_df).encode(x=alt.X('Date', title=None), y='감성지수').interactive()
    b = alt.Chart(filtered_df).encode(x=alt.X('Date', title=None), y='KOSPI', color=alt.value('red')).interactive()
    st.altair_chart((a.mark_line() + b.mark_line()).resolve_scale(y='independent').
                    configure_axisRight(labelColor='red', titleColor='red').
                    configure_axisLeft(labelColor='#1f77b4', titleColor='#1f77b4'),
                    use_container_width=True)
    ...
    st.markdown(
        '감성지수는 주식시장의 기대변동성 지표와 역의 관계가 존재합니다. 애널리스트 실적전망은 추세적인데 반해, 애널리스트의 감성은 시장의 센티멘트에 영향을 주고 받기 때문에 민감하게 반응합니다. ')
    a = alt.Chart(filtered_df).encode(x=alt.X('Date', title=None), y='감성지수').interactive()
    b = alt.Chart(filtered_df).encode(x=alt.X('Date', title=None), y='VKOSPI', color=alt.value('red')).interactive()
    st.altair_chart((a.mark_line() + b.mark_line()).resolve_scale(y='independent').
                    configure_axisRight(labelColor='red', titleColor='red').
                    configure_axisLeft(labelColor='#1f77b4', titleColor='#1f77b4'),
                    use_container_width=True)
    ...
    a = alt.Chart(filtered_df).encode(x=alt.X('Date', title=None), y=alt.Y(tickerSymbol, scale=alt.Scale(domain=(0,80)))).interactive()
    b = alt.Chart(filtered_df).encode(x=alt.X('Date', title=None), y=alt.Y('감성지수', scale=alt.Scale(domain=(0,80))), color=alt.value('red')).interactive()
    st.altair_chart((a.mark_line() + b.mark_line()).resolve_scale(y='independent').
                    configure_axisRight(labelColor='red', titleColor='red').
                    configure_axisLeft(labelColor='#1f77b4', titleColor='#1f77b4'),
                    use_container_width=True)
    ...
    a = alt.Chart(stdzed).encode(x=alt.X('Industry', title=None), y=alt.Y('3년 평균', scale=alt.Scale(domain=(0, 70)))).interactive()
    b = alt.Chart(stdzed).encode(x=alt.X('Industry', title=None), y=alt.Y('현재', scale=alt.Scale(domain=(0, 70))),
                                 color=alt.value('red')).interactive()
    st.altair_chart(
...
if option == 'Risk Appetite Index':
    ...
    st.subheader('1. 글로벌 투자자산군 특성치와 CML')
    st.markdown(
        '대부분의 투자자들은 위험에 대해 회피성향을 가지며 효율적인 시장 하에서 추가적인 리스크에 대한 보상인 리스크 프리미엄은 일관되게 우상향하는 자본시장선(CML)을 형성합니다. 그러나 효율적인 시장이라면 어떤 특정한 자산의 높은 위험조정성과는 오래 지속되기 힘들며 자산가격 상승압력을 받아 향후 기대수익률이 낮아지게 됩니다. 기간선택 slider의 end date를 조정하면 자산군 특성치와 CML의 변화를 확인할 수 있습니다.')

    end_date = filtered_df.tail(1)['Date']
    st.markdown('%s 기준' % (end_date.item().strftime("%m/%d/%Y")))

    asset = pd.Series(
        ["Global E", "DM E", "EM E", "KOSPI", "US Trsy", "EM_Trsy", "Comdty", "Gold", "HY", "Credit", "REIT"])
    assets = pd.DataFrame(asset).T
    assets.columns = asset

    returns = pd.DataFrame(filtered_df.tail(1).iloc[:, 29:40])
    returns.columns = asset
    risk = pd.DataFrame(filtered_df.tail(1).iloc[:, 41:52])
    risk.columns = asset
    profile = pd.concat([assets, risk, returns], axis=0).T
    ...
    c = a.mark_text(dx=30).encode(text='asset')

    st.altair_chart((a.mark_circle(size=60) + b.mark_line() + c), use_container_width=True)

    st.subheader('2. CML 그래프 기울기')
    st.markdown(
    ...
    h = alt.Chart(short).encode(x=alt.X('Date', title=None), y=alt.Y('coef', title="점선: 95% 신뢰구간")).interactive()
    i = alt.Chart(short).encode(x=alt.X('Date', title=None), y=alt.Y('conf_L')).interactive()
    j = alt.Chart(short).encode(x=alt.X('Date', title=None), y=alt.Y('conf_H')).interactive()

    st.altair_chart(h.mark_line() + i.mark_line(strokeDash=[3, 3]) + j.mark_line(strokeDash=[3, 3]),
                    use_container_width=True)
    st.markdown(download_csv('Filtered Data Frame', filtered_df), unsafe_allow_html=True)

    st.subheader('3. RAI와 MSCI ACWI')
    a = alt.Chart(filtered_df).mark_area(opacity=1).encode(x=alt.X('Date', title=None),
                                                           y=alt.Y('MIN-MAX norm', title='RAI')).interactive()
    b = alt.Chart(filtered_df).mark_area(opacity=0.6).encode(x=alt.X('Date', title=None), y='MSCI AC World',
                                                             color=alt.value('red')).interactive()
    st.altair_chart((a.mark_line() + b.mark_line()).resolve_scale(y='independent').
                    configure_axisRight(labelColor='red', titleColor='red').
    ...
    a = alt.Chart(filtered_df).mark_area(opacity=1).encode(x=alt.X('Date', title=None),
                                                           y=alt.Y('MIN-MAX norm', title='RAI')).interactive()
    b = alt.Chart(filtered_df).mark_area(opacity=0.6).encode(x=alt.X('Date', title=None), y='CBOE VIX',
                                                             color=alt.value('red')).interactive()
    st.altair_chart((a.mark_line() + b.mark_line()).resolve_scale(y='independent').
                    configure_axisRight(labelColor='red', titleColor='red').
                    configure_axisLeft(labelColor='#1f77b4', titleColor='#1f77b4'), use_container_width=True)
    ...
    b = alt.Chart(filtered_df).mark_area(opacity=0.6). \
        encode(x='Date', y='AAII Sentiment Bullish', color=alt.value('red')).interactive()
    st.altair_chart((a.mark_line() + b.mark_line()).resolve_scale(y='independent').
                    configure_axisRight(labelColor='red', titleColor='red').
                    configure_axisLeft(labelColor='#1f77b4', titleColor='#1f77b4'), use_container_width=True)
